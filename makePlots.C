 ...
 *  2012IFF
 *
 *  Created by Keith Landry on 4/23/14.
 *  UCLA
 *
 */
...
void makePlots(){
	...
	/*
	 //BINING SCHEME FROM Wednesday, May 28, 2014 3:31:14 PM
	 const int nPtBins    = 8;
	 const int nEtaBins   = 6;
	 const int nMassBins  = 9;
	 const int nPhisrBins = 32;    //moved from 16 to 32 on 4/29/2014
	 
	 double    ptBinStart[nPtBins] = {3.0, 3.7, 4.15, 4.63, 5.19, 5.87, 6.81, 8.45};
	 double    ptBinEnd[nPtBins]   = {3.7, 4.15, 4.63, 5.19, 5.87, 6.81, 8.45, 50};  //Anselm used 1000.0 as last point
	...
	//double    cosThetaBinStart[nCosThetaBins] = {0,1.0,1.3,1.5,1.8,2.0};
	//double    cosThetaBinEnd[nCosThetaBins]   = {1.0,1.3,1.5,1.8,2.0,3.14159};
	...
	//TFile* infile = new TFile("/Users/keithlandry/Desktop/Research/2012IFF/rootFiles/pairsFrom4_2Fullrange.root");
	//TFile* infile = new TFile("/Users/keithlandry/Desktop/Research/2012IFF/rootFiles/pairsFrom4_2FullrangeHiEta.root");
	//TFile* infile = new TFile("/Users/keithlandry/Desktop/Research/2012IFF/rootFiles/pairsFrom4_2FullrangeLoEta.root");
	//TFile* infile = new TFile("/Users/keithlandry/Desktop/Research/2012IFF/rootFiles/sameSignFrom4_25Fullrange.root");
	//TFile* infile = new TFile("/Users/keithlandry/Desktop/Research/2012IFF/rootFiles/pairsFrom4_2FullrangeBlueEta.root");
	//TFile* infile = new TFile("/Users/keithlandry/Desktop/Research/2012IFF/rootFiles/pairsFrom4_2FullrangeYellowEta.root");
	//TFile* infile = new TFile("/Users/keithlandry/Desktop/Research/2012IFF/rootFiles/32bins_pairs_4_2.root");
	//TFile* infile = new TFile("/Users/keithlandry/Desktop/Research/2012IFF/rootFiles/blueEtaGT0_pairs_4_2.root");
	//TFile* infile = new TFile("/Users/keithlandry/Desktop/Research/2012IFF/rootFiles/newBins_4_2.root");
	//TFile* infile = new TFile("/Users/keithlandry/Desktop/Research/2012IFF/rootFiles/loEta32bins_4_2.root");
	//TFile* infile = new TFile("/Users/keithlandry/Desktop/Research/2012IFF/rootFiles/5_19Full.root");
	//TFile* infile = new TFile("/Users/keithlandry/Desktop/Research/2012IFF/rootFiles/5_19_binning_5_28.root");
//	TFile* infile = new TFile("/Users/keithlandry/Desktop/Research/2012IFF/rootFiles/5_19_binning_5_28LoEta.root");
	//TFile* infile = new TFile("/Users/keithlandry/Desktop/Research/2012IFF/rootFiles/5_19_binning_6_25LoEta.root");
	//TFile* infile = new TFile("/Users/keithlandry/Desktop/Research/2012IFF/rootFiles/5_19_binning_6_25rad2FullEta.root");
	//TFile* infile = new TFile("/Users/keithlandry/Desktop/Research/2012IFF/rootFiles/Theta_nearRho_hiEta_9_9.root");
	//TFile* infile = new TFile("/Users/keithlandry/Desktop/Research/2012IFF/rootFiles/Theta_055-150M_4-6Pt_hiEta_9_9.root");
	//TFile* infile = new TFile("/Users/keithlandry/Desktop/Research/2012IFF/rootFiles/Theta_055-150M_6-10Pt_hiEta_9_9.root");
	//TFile* infile = new TFile("/Users/keithlandry/Desktop/Research/2012IFF/rootFiles/Theta_000-060M_6-10Pt_hiEta_9_9.root");
	//TFile* infile = new TFile("/Users/keithlandry/Desktop/Research/2012IFF/rootFiles/Theta_080-300M_6-10Pt_hiEta_9_9.root");
	//TFile* infile = new TFile("/Users/keithlandry/Desktop/Research/2012IFF/rootFiles/cone03AsymsFullEta_9_9_JP1JP2.root");
	...
	for (int i=0; i<nMassBins; i++)
	{
		...
		char titleMass[150];
		sprintf(titleMass, "%.1f < P_{T}^{#pi^{+}#pi^{-}} < %.1f | %.1f < M_{inv}^{#pi^{+}#pi^{-}} < %.1f | %.1f < #eta^{#pi^{+}#pi^{-}} < %.1f", lowLimitPt, hiLimitPt, massBinStart[i], massBinEnd[i], lowLimitEta, hiLimitEta);
	...
	for (int i=0; i<nPtBins; i++)
	{
		...
		char titlePt[150];
		sprintf(titlePt, "%.1f < P_{T}^{#pi^{+}#pi^{-}} < %.1f  %.1f < M_{inv}^{#pi^{+}#pi^{-}} < %.1f  %.1f < #eta^{#pi^{+}#pi^{-}} < %.1f", ptBinStart[i], ptBinEnd[i], lowLimitMass, hiLimitMass, lowLimitEta, hiLimitEta);
	...
	for (int i=0; i<nCosThetaBins; i++)
	{
		...
		sprintf(titleCosTheta, "%.1f < P_{T}^{#pi^{+}#pi^{-}} < %.1f  %.1f < M_{inv}^{#pi^{+}#pi^{-}} < %.1f  %.1f < #eta^{#pi^{+}#pi^{-}} < %.1f", cosThetaBinStart[i], cosThetaBinEnd[i], lowLimitMass, hiLimitMass, lowLimitEta, hiLimitEta);
	...
	*/
	...
	if (vsEta)
	{
		for (int i=0; i<nEtaBins; i++)
		{
			...
			char titleEta[150];
			sprintf(titleEta, "%.1f < P_{T}^{#pi^{+}#pi^{-}} < %.1f  %.1f < M_{inv}^{#pi^{+}#pi^{-}} < %.1f  %.1f < #eta^{#pi^{+}#pi^{-}} < %.1f", lowLimitPt, hiLimitPt, lowLimitMass, hiLimitMass, etaBinStart[i], etaBinEnd[i]);
	...
	gAsymCosTheta->GetXaxis()->SetTitle("#theta_{#pi^{+}#pi^{-}}");
	gAsymCosTheta->GetYaxis()->SetTitle("A_{UT}");
	
	char asymCosThetaTitle[150];
	sprintf(asymCosThetaTitle, "Asymmetry VS P_{T}^{#pi^{+}#pi^{-}}        (%.1f < M_{inv}^{#pi^{+}#pi^{-}} < %.1f , %.1f < #eta^{#pi^{+}#pi^{-}} < %.1f)", lowLimitMass, hiLimitMass, lowLimitEta, hiLimitEta);
	...
	gAsymPt->Draw("AP");
	gAsymPt->GetXaxis()->SetTitle("P_{T}^{#pi^{+}#pi^{-}}");
	gAsymPt->GetYaxis()->SetTitle("A_{UT}");
	
	char asymPtTitle[150];
	sprintf(asymPtTitle, "Asymmetry VS P_{T}^{#pi^{+}#pi^{-}}        (%.1f < M_{inv}^{#pi^{+}#pi^{-}} < %.1f , %.1f < #eta^{#pi^{+}#pi^{-}} < %.1f)", lowLimitMass, hiLimitMass, lowLimitEta, hiLimitEta);
	...
	gAsymMass->Draw("AP");
	gAsymMass->GetXaxis()->SetTitle("M_{inv}^{#pi^{+}#pi^{-}}");
	gAsymMass->GetYaxis()->SetTitle("A_{UT}");
	
	char asymMassTitle[150];
	sprintf(asymMassTitle, "Asymmetry VS M_{inv}^{#pi^{+}#pi^{-}}        (%.1f < P_{T}^{#pi^{+}#pi^{-}} < %.1f , %.1f < #eta^{#pi^{+}#pi^{-}} < %.1f)", lowLimitPt,hiLimitPt,lowLimitEta,hiLimitEta);
	...
	if (vsEta)
	{
		...
		gAsymEta->Draw("AP");
		gAsymEta->GetXaxis()->SetTitle("#eta^{#pi^{+}#pi^{-}}");
		gAsymEta->GetYaxis()->SetTitle("A_{UT}");
		
		char asymMassTitle[150];
		sprintf(asymMassTitle, "Asymmetry VS #eta^{#pi^{+}#pi^{-}}        (%.1f < P_{T}^{#pi^{+}#pi^{-}} < %.1f , %.1f < M_{inv}^{#pi^{+}#pi^{-}} < %.1f)", lowLimitPt,hiLimitPt,lowLimitMass,hiLimitMass);
		gAsymEta->SetTitle();		
	}
	...
	gChi2Mass->GetXaxis()->SetTitle("M_{inv}^{#pi^{+}#pi^{-}}");
	...
	gChi2Pt->GetXaxis()->SetTitle("P_{T}^{#pi^{+}#pi^{-}}");
	...
	if (vsEta)
	{
		...
		gChi2Eta->GetXaxis()->SetTitle("#eta^{#pi^{+}#pi^{-}}");
	...
	//PRINT OUT VALUES 
	string suffix = "3_26_15_JP12_FullEta";
