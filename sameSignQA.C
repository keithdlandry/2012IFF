 ...
 *  2012IFF
 *
 *  Created by Keith Landry on 5/13/15.
 *  UCLA
 *
 */
...
void sameSignQA(){
  ...
  //______________________________
	...
	TTree* sameSignTree = infile->Get("sameSignTree");
	sameSignTree->SetBranchAddress("sameSignPair", &pair1);
	//______________________________
	
	
	cout << sameSignTree->GetEntries() << endl;
	sameSignTree->GetEntry(0);
  ...
  int withJP12 = 0;
	...
	for (int iPair = 0; iPair < sameSignTree->GetEntries(); iPair++)
	{
		if (iPair%10000 == 0) {cout << "processing pair number " << iPair << endl;}
		//if (iPair == 1000000){break;}
    
		
		sameSignTree->GetEntry(iPair);
         ...
         //*/
				...
				for (int ieta=0; ieta<20; ieta++)
				{
					...
					if (sum.Eta() > etastart[ieta] && sum.Eta() < etaend[ieta])
					{
						...
						hNsigma[ieta]->Fill(pair1->getPion1()->nSigmaPion());
						hNsigma[ieta]->Fill(pair1->getPion2()->nSigmaPion());
  ...
  TFile* outFile = new TFile("/star/u/klandry/ucladisk/2012IFF/pairQA/sameSign_4_14_InvarMassRad03.root","recreate");
