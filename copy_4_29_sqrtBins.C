...
void sqrtBins(){
	...
	string outFileName = "./resultsNew_4_22/pol1.root"; 
	...
	for (int iPair = pionStarNumber; iPair < pairTree->GetEntries(); iPair++)
	{
		...
		if (pair1->withinRadius(0.05, 0.3))
		{
			...
			if (triggerFired)
			{
				...
				sumY.RotateY(PI);
				/*
				double dcaPlus  = pair1->piPlus()->dca((pair1->piPlus()->vertexIndex())).mag();
				double dcaMinus = pair1->piMinus()->dca((pair1->piMinus()->vertexIndex())).mag();
