...
//double    ptBinStart[nPtBins] = {3.0, 4.0, 5.0, 6.0 ,7.0, 8.0, 10.0,};
//double    ptBinEnd[nPtBins]   = {4.0, 5.0, 6.0, 7.0, 8.0, 10.0, 50.0};  //Anselm used 1000.0 as last point
...
//double    massBinStart[nMassBins] = {0.0, 0.4, 0.6, 0.72, 0.82, 0.92, 1.5};
//double    massBinEnd[nMassBins]   = {0.4, 0.6, 0.72, 0.82, 0.92, 1.5, 100.5};


double    etaBinStart[nEtaBins] = {-2,-0.5,0.0,0.5};
double    etaBinEnd[nEtaBins]   = {-0.5,0.0,0.5, 2};

//double    etaBinStart[nEtaBins] = {-1.4, -0.9, -0.4, 0.0, 0.4, 0.9};
//double    etaBinEnd[nEtaBins]   = {-0.9, -0.4, 0.0, 0.4, 0.9, 1.4};
...
//double    cosThetaBinStart[nCosThetaBins] = {-1,-0.6,-.2,0.2,.6};
//double    cosThetaBinEnd[nCosThetaBins]   = {-.6,-.2,.2,.6,1};

//double    cosThetaBinStart[nCosThetaBins] = {0,.628,1.257,1.885,2.513};
//double    cosThetaBinEnd[nCosThetaBins]   = {.628,1.257,1.885,2.513,3.14159};

double    cosThetaBinStart[nCosThetaBins] = {0,1.257,1.6,1.8,2};
double    cosThetaBinEnd[nCosThetaBins]   = {1.257,1.6,1.8,2,3.14159};

//double    cosThetaBinStart[nCosThetaBins] = {0,1.0,1.2,1.4,1.6,1.8,2.0};
//double    cosThetaBinEnd[nCosThetaBins]   = {1.0,1.2,1.4,1.6,1.8,2.0,3.14159};
...
//	TFile* infile = new TFile("/star/u/klandry/ucladisk/2012IFF/schedOut_9_9/allPairs_9_9.root");
	...
	string outFileName = "./results_3_26_15/cone03AsymsFullEta_3_26_15_JP1JP2.root"; 
  ...
  //______________________________
	...
	*/
	...
	/*
	 Each Variable we want to look at needs several histograms to calculate the asymmetry.
	 We want to store the numberUp and numberDown for each bin in each variable. 
	 These histograms will be filled and used to create the asymmetry histogram. 
	 Thus each bin in each variable needs an asymmetry histogram. (Aut) 
	 We also need the determine the polarization of each bin for each variable as 
	 this also goes into creating the asymmetry histogram. Not only this but we 
	 also want a seperate polarization for each phiSR bin we have in the asymmetry
	 histograms. This is why the array of polarization histograms is multidemensional.
	 Finally we want to know the average values of each bin so we have something
	 to plot the asymmetry against. So we aslo create histograms to hold each
	 variable broken up into the corresponding bins. 
	*/
  ...
  // -- MASS -- //
	...
	//double lowLimitMass  = .8;
	//double hiLimitMass   = 3;
	//double lowLimitPt    = 6;
	//double hiLimitPt     = 10;
  ...
  //random number for randomizing assignment.
	//set seed to zero to guaranty unique numbers each time.
	TRandom3 rAssign;
	rAssign.SetSeed(0);
	...
	for (int iPair = pionStarNumber; iPair < pairTree->GetEntries(); iPair++)
	{
		...
		//if (iPair == pionStarNumber+1000000){break;}
      ...
      ///*
        ...
        //option for randomizing pion assignment
        if (randomizePiAssignment)
        {
          double randomAss = r.Uniform(0, 1);
          
          //switch order if random is less than 0.5
          if (randomAss >= 0 && randomAss < 0.5)
          {
            pair1->setPiPlusPiMinus(pair1->piMinus(), pair1->piPlus(), pair1->spinBit());
          }
        }// end rand ass
				...
				if (passInitialCut_B && useBlueBeam)
				{
					...
					{						
						...
						//cout << spinVect.Angle(sumB.Vect()) << endl;
						...
						//fillCorrectCosThetaBin(hNumberUp_CosTheta, pair1->theta(), pair1->phiSR('b'), polOfBin_CosTheta, polarizationOfFill_B[blueFillNo], hCosTheta);
					  //fillCorrectSPhAngleBin(hNumberUp_sPhAngle, spinVect.Angle(sumB.Vect()), pair1->phiSR('b'), polOfBin_sPhAngle, polarizationOfFill_B[blueFillNo], hSPhAngle);
					...
					{						
						...
						//fillCorrectCosThetaBin(hNumberDown_CosTheta, pair1->theta(), pair1->phiSR('b'), polOfBin_CosTheta, polarizationOfFill_B[blueFillNo], hCosTheta);
					  //fillCorrectSPhAngleBin(hNumberDown_sPhAngle, spinVect.Angle(sumB.Vect()), pair1->phiSR('b'), polOfBin_sPhAngle, polarizationOfFill_B[blueFillNo], hSPhAngle);
				...
				if (passInitialCut_Y && useYellowBeam)
				{
					...
					if (pair1->spinBit() == 5 || pair1->spinBit() == 9)
					{
						...
						//fillCorrectCosThetaBin(hNumberDown_CosTheta, pair1->theta(), pair1->phiSR('y'), polOfBin_CosTheta, polarizationOfFill_Y[yellowFillNo], hCosTheta);
						//fillCorrectSPhAngleBin(hNumberDown_sPhAngle, spinVect.Angle(sumY.Vect()), pair1->phiSR('y'), polOfBin_sPhAngle, polarizationOfFill_Y[yellowFillNo], hSPhAngle);
  ...
  /*
	for (int i=0; i<nS_Ph_angleBins; i++)
	{
		cout << hNumberUp_sPhAngle[i]->GetEntries() << "  " << hNumberDown_sPhAngle[i]->GetEntries() << endl;
		cout << polOfBin_sPhAngle[i][8]->GetEntries() << endl;
	}
*/
