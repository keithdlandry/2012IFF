 ...
 *  2012IFF
 *
 *  Created by Keith Landry on 2/19/14.
 *  UCLA
 *
 */
	...
	for (int iPair = 0; iPair < pairTree->GetEntries(); iPair++)
	{
		...
		if (pair1->withinRadius(0.05, 0.3))
		{
			...
			if (triggerFired)
			{
				...
				if (randomSpin >=0 && randomSpin <0.25)  {randomSpinBit = 5;}
				if (randomSpin >=0.25 && randomSpin <0.5){randomSpinBit = 6;}
				if (randomSpin >=0.5 && randomSpin <0.75){randomSpinBit = 9;}
				if (randomSpin >=0.75 && randomSpin <1.0){randomSpinBit = 10;}
