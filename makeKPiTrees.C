 ...
 *  2012IFF
 *
 *  Created by Keith Landry on 4/17/15.
 *  UCLA
 *
 */
...
void makeKPiTrees(string fileList, string jobID){
  ...
  //pionPairTreemaker
	if (makePairs)
	{
    dihadronPairTreeMaker* kPiTreeMaker = new dihadronPairTreeMaker(muDstMaker);
		string outFileName;
		outFileName  = "run";
		outFileName += runNumber;
		outFileName += "_";
		outFileName += jobID;
		outFileName += "_dihadronTree.root";
		
		kPiTreeMaker->setOutputFile(outFileName.c_str());
    cout << "kiPiTree file name: " << outFileName << endl;	
	}
